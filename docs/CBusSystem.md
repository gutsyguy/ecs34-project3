...
The `CBusSystem` class is an abstract interface representing a collection of bus stops and routes. It provides methods to access stops by index or ID, and routes by index or name.
...
```cpp
void PrintBusSystemSummary(std::shared_ptr<CBusSystem> busSystem) {
    std::cout << "Stops: " << busSystem->StopCount() << std::endl;
    std::cout << "Routes: " << busSystem->RouteCount() << std::endl;
}
```
